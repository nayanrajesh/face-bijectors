...
def IsReduceable(LHS,RHS,m):
	if RHS == [] or LHS == []:
		return False
	else:
		for Term in RHS:
			if Factorize(LHS,Pairs,Term,m):
				return True
	return False
...
def Rewrite(Pairs, LHS,RHS,m, pnt=None):
	if not IsReduceable(LHS,RHS,m):
		return None
	if LHS == [] and RHS == []:
		return []		
	else:
		for Term in list(set(RHS)):
			Fax = Factorize(LHS,Pairs,Term,m)
			for Fac in Fax:
				(L,R) = Replace(LHS,RHS,Fac)
				if Rewrite(Pairs,L,R,m):
					return True
	return False 
